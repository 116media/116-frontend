...
import { FAVORITES_COLLECTION_PARAM } from "@/modules/favorites/presentation/constants/favoritesCollections";
import {
    BookmarkIcon,
    HeartIcon,
    ListVideoIcon,
...
/**
 * The leading icon for each favorites collection, keyed by collection name.
 */
...
/**
 * One selectable collection tab, with its total item count.
 *
 * @interface FavoriteCollectionTab
 ...
 */
export interface FavoriteCollectionTab {
    key: string;
    count?: number;
}

/**
 * Props for FavoriteCollectionTabs.
 *
 * @interface FavoriteCollectionTabsProps
 * @property {ReadonlyArray<FavoriteCollectionTab>} collections - The route's allow-listed collections (max four), in display order.
 * @property {string} active - The currently active collection, already normalized by the container.
 */
export interface FavoriteCollectionTabsProps {
    collections: ReadonlyArray<FavoriteCollectionTab>;
    active: string;
}
 ...
 */
export function FavoriteCollectionTabs({ collections, active }: FavoriteCollectionTabsProps) {
