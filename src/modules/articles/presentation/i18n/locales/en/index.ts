 ...
 * bundle covering `card`, `grid` and `filters`), mirroring the shared locale barrel.
 * The composition root nests it under the `articles` key, so keys resolve as
 * `t("articles.home.categories")` or `t("articles.card.read")`. Must stay key-aligned
