 ...
 *
 * @description
 * The bottom-anchored, locale-aware activity row inside the body: an optional relative
 * timestamp, own comment/share counts, share channel, and own star rating. Renders only
 * the pieces it is given; dates and counts go through the shared formatters/plurals.
 */
export function FavoriteCardMeta({
    ...
    return (
        <div
            className={cn(
                "mt-auto flex justify-between items-center gap-x-4 gap-y-1 text-muted-foreground text-xs",
                className
            )}
        >
            {relative && (
