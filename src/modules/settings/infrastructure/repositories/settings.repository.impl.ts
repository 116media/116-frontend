...
import { SettingsMapper } from "@/modules/settings/infrastructure/mappers/settings.mapper";
 ...
 * get/update and password change. The user DTO is mapped by the shared `UserMapper`.
...
export class SettingsRepositoryImpl implements ISettingsRepositoryPort {
    ...
    async getProfile(): Promise<Result<IProfile>> {
        try {
            ...
            return ok(UserMapper.userFromDto(response.data.user));
        } catch (error) {
    ...
    async changePassword(
        ...
        try {
            ...
            return ok(SettingsMapper.actionFromDto(response.data));
        } catch (error) {
            return err(ProblemMapper.toFailure(error));
        }
    }
